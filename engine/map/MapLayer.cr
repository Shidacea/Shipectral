class MapLayer < SF::Transformable
  ...
  def dup
    new_map_layer = MapLayer.new(@width, @height, @view_width, @view_height, @tile_width, @tile_height)
    if tileset = @tileset
...
def setup_ruby_map_layer_class(rb)
  Anyolite.wrap(rb, MapLayer, under: SF, verbose: true, connect_to_superclass: true)
end